...
export const storageService = {
  generateReferenceId: async (projectId: string, category: 'Defect' | 'Risk'): Promise<string> => {
    const counters = await store.getItem<ReferenceCounters>(COUNTERS_KEY) || {};
    ...
    await store.setItem(COUNTERS_KEY, counters);
  ...
  saveDraft: async (inspection: Inspection): Promise<void> => {
    try {
      ...
      await store.setItem(DRAFTS_KEY, drafts);
    } catch (error) {
  ...
  getDrafts: async (): Promise<Inspection[]> => {
    try {
      const drafts = await store.getItem<Inspection[]>(DRAFTS_KEY);
      return drafts || [];
    } catch (error) {
  ...
  saveAutoSave: async (inspection: Partial<Inspection>): Promise<void> => {
    try {
      await store.setItem(AUTO_SAVE_KEY, inspection);
    } catch (error) {
  ...
  getAutoSave: async (): Promise<Partial<Inspection> | null> => {
    try {
      return await store.getItem(AUTO_SAVE_KEY);
    } catch (error) {
  ...
  getPhotos: async (projectId: string): Promise<Photo[]> => {
    try {
      ...
      for (const key of photoKeys) {
        const photo = await store.getItem<Photo>(key);
